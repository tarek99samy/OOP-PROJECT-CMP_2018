...
//void window::DrawRhombus(const int iX1, const int iY1, const int iX2, const int iY2, const int iX3, const int iY3,const int iX4, const int iY4, const drawstyle dsStyle) {
//
//	HRGN rgnTemp;
//	POINT ptVertices[4] = {{iX1, iY1}, {iX2, iY2}, {iX3, iY3},{iX4,iY4}};
//
//	ProcessMessage(); // Kludge
//    switch(dsStyle) {
//      case FILLED:
//		ChangeDrawStyle(FILLED);	    
// 	    if(Polygon(dcActive, ptVertices, 4) != TRUE) {
//            cout << "Fatal Error: Failed to draw filled style in DrawTriangle!" << endl;	        	    		    
// 	    }
// 	    RestoreDrawStyle();
//		break;
...
//		
//	  case INVERTED:
//		rgnTemp = CreatePolygonRgn(ptVertices, 4, WINDING);
// 	    if(InvertRgn(dcActive, rgnTemp) != TRUE) {
//            cout << "Fatal Error: Failed to draw inverted style in DrawTriangle!" << endl;	        	    		     	    
// 	    }
//		DeleteObject(rgnTemp);
...
void window::DrawImage(const image &imgThis, const int iX, const int iY, const int iWidth, const int iHeight) {
  ...
  DrawImage(&imgThis, iX-10, iY, iWidth-15, iHeight);
